...
> 条件审查员：当改动涉及 UI/交互时自动加入 `webapp-consistency-audit` + `playwright-e2e-testing`（E2E 视觉+功能测试），涉及数据/API 时加入 `pre-release-test-reviewer`。审查员选择由 `routing/phase-d-reviewer-selector.yaml` 配置驱动。
...
| **D** | D.1 Implement → D.2 Review → D.3 Accept | `full-stack-developer` → reviewers (incl. `playwright-e2e-testing` for UI changes) | **Gate 3** Acceptance |
