...
此外，enforcer 在 UserPromptSubmit 时执行两项额外检查：
...
> **背景**：在 LLM-first orchestration 的前提下，主 Orchestrator 的注意力是稀缺资源。V5.0 的补强原则是：不加厚主编排层，而是在 sub-agent 产出的 schema 里补充结构化信号，让 Orchestrator 在关键判断点少推理、早发现。

三项变更全部为加法，不改现有 Phase 骨架、不改 Gate 语义、不新增子协议文件。

### 1. `completion_status`（change-package + review-report 通用）
...
```yaml
debug_closure:                            # 条件出现：task_type in [bugfix, hotfix]
  all_symptoms_explained: true | false
  secondary_root_cause_checked: true | false
  adjacent_impact_checked: true | false
  verification_scope: local | cloud | both
```

**作用**：closure check，不是 investigation 协议。让 FSD 在宣告"修好了"之前，显式回答 4 个固定收尾问题。`false` 是合法答案——reviewer 会在审查中跟进。字段固定，不扩展。

### 3. `verification_boundary`（change-package 条件字段）

```yaml
verification_boundary:
  verified: ["TS 编译", "主流程手动走通"]   # ≤ 3 条，每条 ≤ 15 字
  unverified: ["飞书数据对象映射"]          # ≤ 3 条，每条 ≤ 15 字
  unverified_reason: "本地无飞书沙箱"       # 1 句，≤ 40 字
```

**触发条件**：`execution_plan` 中任一 scope 有 `host_target` 非空，或 `cloud_validation_required: true`，或 change-package 中 `delivery_readiness` 存在。三者共同语义是"存在本地验证边界之外的交付面"——纯本地 repo 改动无此字段。

**作用**：Gate 3 汇总声明。与 `tests_run` 是 summary-detail 关系，不重复。字段数量和字符数上限是强制约束。Gate 3 展示时条件呈现。
