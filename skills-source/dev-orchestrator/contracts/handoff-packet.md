...
project_design_context:           # 可选；相关文件不存在时省略此字段
  ...
  must_read_refs:                    # 项目级设计文档——FSD 在 UI scope 时必须逐个阅读并在 change-package 输出消费回执
    - "{project_path}/DESIGN-SPEC.md"
    - "{project_path}/design/implementation-governance.md"
    - "{project_path}/design/page-patterns.md"
    - "{project_path}/design/component-guidelines.md"
    - "{project_path}/client/src/tailwind-theme.css"
  optional_refs:                     # 按需参考；FSD 不强制阅读，不要求消费回执
    - "{project_path}/design/visual-system.md"
    - "{project_path}/design/dark-mode.md"
    - "{project_path}/design/visual-interaction-audit-checklist.md"
  # 填写规则：
  # - orchestrator 逐个检查文件是否存在，只填入**已存在**的路径（不填 null、不填不存在的路径）
  # - must_read_refs 为空列表或整个 project_design_context 省略 = 项目无设计约束
  #   → FSD / reviewer / gate 均不检查设计消费
  # - must_read_refs 非空 = FSD 必须在 change-package 中输出 design_consumption_receipt（见 change-package.md）
```

## project_design_context 使用规则

- **消费方**：frontend-design（Phase C）、**full-stack-developer（Phase D.1，must_read_refs 非空时强制消费）**、component-library-maintainer（Phase C 条件）、webapp-consistency-audit（Phase D.2 条件）、**code-reviewer（Phase D.2，验证 design_consumption_receipt）**
...
- **单一真源**：`project_design_context` 是设计文档引用的唯一结构化来源。implementation-scope "设计规范参考"节、FSD receipt、reviewer 检查、gate 断言全部基于同一组 refs。不另建平行字段
