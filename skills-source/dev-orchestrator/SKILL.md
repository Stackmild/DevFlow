...
详见下方 §Contract: Gate 3 Continuation Protocol。
⚠️ orchestrator **不可**在 Gate 3 后默认进入 ad-hoc 工作模式。不确定走哪条 → 展示五选项。多个独立请求 → 走 Multi-Item 处理协议。

⚠️ **Continuation 不应降低 contract 强度**（V4.3 总原则）：RE-ENTER D 默认要求与首轮 D 阶段同级的 change-package + reviewer handoff + review artifact + Gate 3 decision。不可因"scope 小 / 只改两行 / 续行轮次"自动降级。详见下方 §Contract: Gate 3 Continuation Protocol §总原则。
...
详细步骤见下方 §Protocol: Write-Through Action Templates。
...
详细流程见下方 §Protocol: State Conflict Resolution。
...
前置自检：执行下方 §Protocol: Pre-Gate Self-Check §2.1（PG1-1~6），result=blocked 时不展示 Gate
...
前置自检：执行下方 §Protocol: Pre-Gate Self-Check §2.2（PG2-1~8），result=blocked 时不展示 Gate
...
前置自检：执行下方 §Protocol: Pre-Gate Self-Check §2.3（PG3-1~13），result=blocked 时不展示 Gate
